(function($){
    function crateCommentInfo(obj){
        ...
        el = el + "</div><div class='col-md-2'><span class='reply-btn' reply-id='' msg-id='" + obj.id + "'>回复</span></div></div></div><div class='reply-list'>";
    ...
    function createReplyComment(reply){
        var replyEl = "<div class='reply'>" +
            "               <div>" +
            "                   <a href='javascript:void(0)' class='replyname'>"+reply.replyName+"</a>:<a href='javascript:void(0)'>@"+reply.beReplyName+"</a>" +
            "                   <span>"+reply.content+"</span>" +
            "               </div>" +
            "               <p>" +
            "                   <span>"+reply.time+"</span> " +
            "                   <span class='reply-list-btn' reply-id='" + reply.id + "' msg-id='" + reply.msgId + "'>回复</span>" +
            "               </p>" +
            "           </div>";
        return replyEl;
    }
    ...
    function replyClick(el, replyFun){
        var replyBoxEl = "<div class='replybox'>" +
            "                   <textarea cols='80' rows='5' placeholder='来说几句吧......' class='form-control mytextarea hundred2String' ></textarea>" +
            "                   <span class='send' reply-id='" + $(el).attr("reply-id") + "' msg-id='" + $(el).attr("msg-id") + "'>发送</span>" +
            "              </div>";
        el.parent().parent().append(replyBoxEl)
            ...
            if(content != ""){
                ...
                // $(".replybox").remove();
                // parentEl.find(".reply-list").append(replyString).find(".reply-list-btn:last").click(function(){errorDialog("不能回复自己");});
                replyFun($(this).attr("msg-id"), content, $(this).attr("reply-id"));
            }else{
                errorDialog("空内容");
            }
        });
        ...
        if(option.data.length > 0){
            ...
            $(this).append(totalString).find(".reply-btn").click(function(){
                if($(this).parent().parent().find(".replybox").length > 0){
                    ...
                    $(".replybox").remove();
                    replyClick($(this), option.replyContentFun);
                }
            });
